(function (angular, undefined) {
    ...
    /** MarketSharp Changes to Library by NST.
    ---------------------------------------------------
    1. Lines 85-87 were moved up so the Add button displays above the list in-case the list is big. <button type="button" class="btn btn-primary btn-block add-button" ng-click="add()">{{addText}}</button>
    2. Lines 287-288 in configChildScope() were added so that after clicking Add, the search term is cleared and the focus is set on the selectedItem that was added.
    3. Lines 215-220 were modified as the autofocus attribute was added per request by Axel; the creator of the customSelect directive.  
       The setFocus code is wrapped in a $timeout fct to avoid the $digest errors. 
    4. Lines 82-86 were modified to change the addText displayed as the user types. A placeholder of "Enter Text" was also inserted so as to be more intuitive UX.
    --------------------------------------------------
     **/
            ...
            link: function (scope, elem, attrs, controller) {
                ...
                var itemTemplate = elem.html().trim() || '{{' + (match[2] || match[1]) + '}}',
					...
					'<div class="dropdown-menu">' +
						'<div stop-propagation="click" class="custom-select-search">' +
							'<input class="' + attrs.selectClass + '" type="text" placeholder="Enter Text" autocomplete="off" ng-model="' + searchModel + '" />' +
						'</div>' +
						'<div class="custom-select-action">' +
							(typeof options.onAdd === "function" ?
							"<button type='button' class='btn btn-primary btn-block add-button' ng-click='add()'>{{search && search.text.length ? 'Add \"' + search.text + '\"' : addText }}</button>" : '') +
						'</div>' +
						'<ul role="menu">' +
                ...
                function configChildScope() {
                    ...
                    childScope.add = function () {
                        ...
                        childScope.search = getInitialSearchModel(); //clear the search bar
                        anchorElement.focus(); //added by Nick --> sets focus after add
                    };
